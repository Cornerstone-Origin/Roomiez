                    ...
                    HStack(alignment: .top, spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            HStack(spacing: 6) {
                                ...
                                if chore.groupId != nil {
                                    RotationChip()
                                } else if chore.recurrence != .once {
...
/// Marks chores materialized by the weekly rotation scheduler. Visually
/// distinct from `RecurrenceChip` (different label + icon) so users can
/// tell "this came from a group rotation" apart from "this is a personal
/// recurring chore". Tinted with the secondary brand colour (azure /
/// Blue Bell) so the new feature shows off the new palette.
struct RotationChip: View {
    var body: some View {
        Label("Rotation", systemImage: "person.2.arrow.trianglehead.counterclockwise")
            .font(.cozyTag)
            .lineLimit(1)
            .fixedSize(horizontal: true, vertical: false)
            .padding(.horizontal, 8).padding(.vertical, 3)
            .background(Capsule().fill(Theme.Palette.azure.opacity(0.16)))
            .overlay(Capsule().stroke(Theme.Palette.azure.opacity(0.40), lineWidth: 0.5))
            .foregroundStyle(Theme.Palette.azure)
    }
}
